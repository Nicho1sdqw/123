...
close all
warning off
%% Step1：读取数据文件   3d上得到绿色的周围点 和 初始的中心点。
filpath = './data/';
file_name = 'point_filtered_mono6-3';
...
%% %识别圆，套用函数
[centers1,radii1] = imfindcircles(I,[2 15],'ObjectPolarity','dark','Sensitivity',1.00,'Method','twostage');
disp('识别圆成功')
...
y=min(Location(:,2)):1:max(Location(:,2));
z=min(Location(:,3)):1:max(Location(:,3));
I=zeros(length(z)-1,length(y)-1);
for i=1:length(z)-1
    for j=1:length(y)-1
        a1=[];a2=[];a3=[];a4=[];
...
%识别圆，同理
[centers2,radii2] = imfindcircles(I,[2 15],'ObjectPolarity','dark','Sensitivity',0.65,'Method','twostage');
%centers返回圆中心点，radii返回对应半径，[2 15]是设置的识别圆的半径范围，0.95是设置的边缘梯度阈值
%'dark'圆形目标比背景暗，'twostage'两阶段圆形 Hough 变换
%检验中心点，同理
c=[];
...
figure(3)
imshow(I)
viscircles(centers2, radii2,'EdgeColor','c');
title('xy面定位')
figure(4)
hold on
...
for i=1:size(xy2,1)
    ...
    ub=[xy1(i,1:2)+0.001,xy1(i,3)+0.0001,1];

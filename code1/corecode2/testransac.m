...
for i = 1:size(xy2,1)
    ...
    hold on;
    %% fit by RANSAC method 
        % RANSAC 算法
        for bbbbb = 1:max_iterations
            % 随机采样
            indices = randperm(size(DataX, 1), 3);
            sample = DataX(indices, :);
            
            % 拟合模型（圆心和半径）
            [center, radius] = fit_circle(sample(:, 1), sample(:, 2));
            
            % 计算内点
            distances = abs(sqrt((DataX(:, 1) - center(1)).^2 + (DataX(:, 2) - center(2)).^2) - radius);
...
save(strcat('./result_circle/', file_name ,'result_RANSAC_boundPoint','.mat'),"P_fitcircle");   
...
for i=1:size(xy2,1)
 ...
 %将结果圆心以及半径写入到result_RANSAC文件中
 save_file = ['result_circle/',file_name ,'result_RANSAC.txt'];
...
function [center, radius] = fit_circle(x, y)
    ...
    b = -(x.^2 + y.^2);
    solution = A\b;
    center = -solution(1:2) ./ 2;
    radius = sqrt((solution(1)^2 + solution(2)^2) / 4 - solution(3));
end

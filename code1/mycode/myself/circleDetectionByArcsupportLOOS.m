function [circles, L,posi] = circleDetectionByArcsupportLOOS(I, Tac, Tni)
%input:
% I: input image
...
% 3648–3652.
    ...
    Tmin = 0.7*Tni;%比例阈值0.5 
    unit_dis_tolerance = 6;%内点距离的容忍差小于max(2,0.5%*minsize)
    normal_tolerance = 12;%法线容忍角度20°
    t0 = clock;
    % if(size(I,3)>1)
    % [candidates,edge,normals] = generateCircleCandidates(I(:,:,3));%B分量
   % [candidates,edge,normals] = generateCircleCandidatesPOELine(I(:,:,3));   
    % else
   ...
   % figure;imshow(edge);
    ...
    % figure;
    % dispimg = drawCircle(I,[candidates(:,1) candidates(:,2)],candidates(:,3));
    % 

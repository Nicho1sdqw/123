...
% 3648–3652.
    ...
    Tmin = 0.3*Tni;%比例阈值0.5 
    unit_dis_tolerance = 4;%内点距离的容忍差小于max(2,0.5%*minsize)
    normal_tolerance = 10;%法线容忍角度20°
    t0 = clock;
    image_shape = size(I);
    % disp(['图像的形状为：',num2str(image_shape)]);
%     if(size(I,3)>1)
%     % [candidates,edge,normals] = generateCircleCandidates(I(:,:,3));%B分量
%    [candidates,edge,normals] = generateCircleCandidatesPOELine(I);   
%     else

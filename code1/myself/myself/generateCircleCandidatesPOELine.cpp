                              ...
                              image_double * modgrad, unsigned int n_bins )
{
  ...
  double com1,com2,com11,com22,gxy3,g13,g23,gx,gy,norm,norm2;
          ...
          /*
           Norm 3 computation
            g3= D+A - (B+c)
              ...
              com1 and com2 are just to avoid 2 additions.
         */
        com11 = in->data[adr+p+1] - in->data[adr+p];
        com22 = in->data[adr+1] - in->data[adr];

        gxy3 =  com22 - com11;
        g13 = gx*gx+gxy3*gxy3;
        g23 = gy*gy - gxy3*gxy3;

        norm2 = gx*gx+gy*gy + 0.0045*(g23/(1.5*g13) );
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int n;//线段数量
    double* out=mylsd(&n, data,imgx,imgy,NULL,NULL,NULL);  //1 要看
  ...
  //  (
	 ...
	 if(pairseglist != NULL)
	 {
		...
		double * centers;
		int      centers_num;
		generateCenterCandidates(pairseglist,distance_tolerance,centers,&centers_num); //2 要看 meanshift方法
		printf("圆心均值漂移并聚类后的圆心数量：%i \n",centers_num);
		double * candidates;
		int  candidates_num;
		generateCircleCandidates(pairseglist,centers,centers_num,distance_tolerance,candidates,&candidates_num);//3 要看 半径mean shift方法

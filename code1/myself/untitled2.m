...
I = binaryImg;
[centers1,radii1] = imfindcircles(binaryImg,[2 10],'ObjectPolarity','dark','Sensitivity',0.7, 'Method','TwoStage');
%可视化
figure(1)
imshow(I)
...
for i=1:size(xy,1)
    ...
    xy_1(i,:)=[xy(i,:),min(xx(:,3))];%用所属边界点的坐标均值更新中心点（三维坐标）
end
...
for i=1:size(xy2,1)
    ...
    X{i,1}=xx;%记录边界点
    XXX00{i,1} = x0;% 记录初始点
    save(strcat('./result_mat/', file_name ,'_3d.mat'),"XXX00","X","xy2",'Location');
end
%可视化
figure(6)
hold on
